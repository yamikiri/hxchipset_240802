...
struct proc_dir_entry *himax_proc_prevent_notch_file;
#endif
#if defined(Lenovo_pen_id_fun)
struct proc_dir_entry *himax_proc_pen_id_file;
#endif
#if defined(Lenovo_support_pen_fun)
struct proc_dir_entry *himax_proc_support_pen_file;
#endif
...
{
	uint8_t direction = hx_s_core_fp._read_panel_direction();
	ssize_t ret = 0;
	...
	{	
		...
		case PANEL_LANDSCAPE_180:
			seq_puts(s, "Panel direction is LANDSCAPE_180\n");
			break;
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	{	
		case '0':
			hx_s_core_fp._set_panel_direction(PANEL_PORTRAIT_0);
			hx_s_ts->direction = PANEL_PORTRAIT_0;			
			break;
		case '1':
			hx_s_core_fp._set_panel_direction(PANEL_LANDSCAPE_90);
			hx_s_ts->direction = PANEL_LANDSCAPE_90;
			break;
		case '2':
			hx_s_core_fp._set_panel_direction(PANEL_LANDSCAPE_180);	
			hx_s_ts->direction = PANEL_LANDSCAPE_180;
			break;
		case '3':
			hx_s_core_fp._set_panel_direction(PANEL_LANDSCAPE_270);
...
#if defined(Lenovo_pen_id_fun)
static int himax_Pen_ID_show(struct seq_file *s, void *v)
{
	uint16_t pen_id_sta = hx_s_core_fp._read_pen_id_mode();
	ssize_t ret = 0;
	int i;
	
	I("%s:pen_id_sta=%02X%02X\n", __func__, pen_id_sta >> 8 & 0xFF, pen_id_sta & 0xFF);

	seq_printf(s,"%02X%02X", pen_id_sta >> 8 & 0xFF, pen_id_sta & 0xFF);
	for(i= 0;i< 7;i++)
		{
			seq_printf(s,"%02X", hx_s_ts->pen_id[i]);
		}
	seq_puts(s, "\n");	
...
static ssize_t himax_Pen_ID_store(struct file *filp,
			const char __user *buff, size_t len, loff_t *data)
{
	char buf[3] = {0};
	uint8_t cmd_tmp[3]={0};
	...
	I("%s cmd is %c\n",__func__,buf[0]);
	sscanf(buf, "%d", &cmd_tmp[0]);
	hx_s_core_fp._set_pen_id_mode(cmd_tmp[0]);
	return len;
}

__CREATE_RW_NODE_HX(Pen_ID);
#endif
...
static int himax_game_mode_show(struct seq_file *s, void *v)
{
	...
	if( hx_s_ts->suspended == false ) {
		game_mode= hx_s_core_fp._read_game_mode();
		switch(game_mode)
		{	
		case 0:
			seq_puts(s, "palm mode is normal\n");
			break;
		case 1:
			seq_puts(s, "palm mode is in pen state\n");
			break;
		case 2:
			seq_puts(s, "palm mode is in game state\n");
			break;	
		default:
			E("%s: cmd  is not match, pls check.\n", __func__);
			seq_puts(s, "palm mode is UNKNOWN\n");
			break;
		}
	} else {
		I("%s now is suspend mode.\n",__func__);
		game_mode =hx_s_ts->game_mode;
	}	

	seq_printf(s,"palm mode= %d\n", game_mode);
	return ret;
}
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	if( hx_s_ts->suspended == false ) {
		I("%s cmd is %c\n",__func__,buf[0]);
		sscanf(buf, "%d", &cmd_tmp[0]);
		hx_s_core_fp._set_game_mode(cmd_tmp[0]);
		hx_s_ts->game_mode = cmd_tmp[0];
	} else {
		I("%s now is suspend mode.\n",__func__);
	}
	return len;
...
#if defined(Lenovo_support_pen_fun)
static int himax_support_pen_show(struct seq_file *s, void *v)
{
	uint8_t support_pen;
	ssize_t ret = 0;

	if( hx_s_ts->suspended == false ) {
		support_pen = hx_s_core_fp._read_support_pen();
		switch(support_pen)
		{	
			case 0:
				if( hx_s_ts->support_pen == 1 ) {
					E("%s fw setting is off,but baseline setting is on\n",__func__);
				} else {
					I("%s pen mode is off \n",__func__);
					seq_puts(s, "pen mode is off\n");
				}			
				break;
			case 1:
				if( hx_s_ts->support_pen == 0 ) {
					E("%s fw setting is on,but baseline setting is off\n",__func__);
				} else {
					I("%s pen mode is on \n",__func__);
					seq_puts(s, "pen mode is on\n");
				}
				break;
			default:
				E("%s: cmd  is not match, pls check.\n", __func__);
				seq_puts(s, "pen mode is UNKNOWN\n");
				break;
		}		
	} else {
		I("%s now is suspend mode.\n",__func__);
		support_pen =hx_s_ts->support_pen;
	}
	
	seq_printf(s,"pen mode= %d\n", hx_s_ts->support_pen);	
	return ret;
}

static ssize_t himax_support_pen_store(struct file *filp,
			const char __user *buff, size_t len, loff_t *data)
{
	char buf[3] = {0};
	uint8_t cmd_tmp[3]={0};
	...
	if( hx_s_ts->suspended == false ) {	
		I("%s cmd is %c\n",__func__,buf[0]);
		sscanf(buf, "%d", &cmd_tmp[0]);
		hx_s_core_fp._set_support_pen(cmd_tmp[0]);
		hx_s_ts->support_pen = cmd_tmp[0];
	} else {
		I("%s now is suspend mode.\n",__func__);
	}
	return len;
...
__CREATE_RW_NODE_HX(support_pen);
#endif
...
{
	ssize_t ret = 0;
	seq_printf(s, "cmd_parm = %d,%d,%d,%d\n", hx_s_ts->cmd_param[0], hx_s_ts->cmd_param[1], hx_s_ts->cmd_param[2], hx_s_ts->cmd_param[3]);
	return ret;
}
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	ret = sscanf(tmp_buf, "%d,%d,%d,%d", &hx_s_ts->cmd_param[0], &hx_s_ts->cmd_param[1], &hx_s_ts->cmd_param[2], &hx_s_ts->cmd_param[3]);

	I("%s:cmd_parm = %d,%d,%d,%d\n", __func__, hx_s_ts->cmd_param[0], hx_s_ts->cmd_param[1], hx_s_ts->cmd_param[2], hx_s_ts->cmd_param[3]);
		
	ret = hx_s_core_fp._set_prevent_notch( hx_s_ts->cmd_param );
	I("%s: ret = %d\n",__func__,ret);
out:
	if (tmp_buf)
		kfree(tmp_buf);
...
int himax_touch_proc_init(void)
{
	...
	if (himax_proc_game_mode_file == NULL) {
		E(" %s: palm mode file create failed!\n", __func__);
		goto fail_6;
	}
#endif
#if defined(Lenovo_norch_fun)	
...
#endif
#if defined(Lenovo_pen_id_fun)
	himax_proc_pen_id_file = proc_create(HIMAX_PROC_PEN_ID_FILE,
				0644, NULL,
				&himax_Pen_ID_ops);
	if (himax_proc_pen_id_file == NULL) {
		E(" %s: pen id file create failed!\n", __func__);
		goto fail_8;
	}
#endif
#if defined(Lenovo_support_pen_fun)
	himax_proc_support_pen_file = proc_create(HIMAX_PROC_SUPPORT_PEN_FILE,
				0644, NULL,
				&himax_support_pen_ops);
	if (himax_proc_support_pen_file == NULL) {
		E(" %s: pen mode file create failed!\n", __func__);
		goto fail_9;
	}
#endif	
	return 0;
...
#endif
#if defined(Lenovo_pen_id_fun)
	remove_proc_entry(HIMAX_PROC_PEN_ID_FILE, NULL);
fail_8:
#endif
#if defined(Lenovo_norch_fun)
	remove_proc_entry(HIMAX_PROC_PREVENT_NOTCH_FILE, NULL);
fail_7:
#endif
#if defined(Lenovo_game_mode_fun)
...
#endif
#if defined(Lenovo_direction_fun)
	remove_proc_entry(HIMAX_PROC_PANEL_DIRECTION_FILE, NULL);
#endif
#if defined(Lenovo_direction_fun)	
fail_5:	
#endif
...
#endif
#if defined(Lenovo_support_pen_fun)
	remove_proc_entry(HIMAX_PROC_SUPPORT_PEN_FILE, NULL);
#endif
#if defined(Lenovo_direction_fun)
	remove_proc_entry(HIMAX_PROC_PANEL_DIRECTION_FILE, NULL);
#endif
