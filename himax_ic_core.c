...
#if defined(Lenovo_direction_fun)
void himax_mcu_set_panel_direction(uint8_t direction)
{
	uint8_t retry_cnt = 0;
	uint8_t tmp_data[DATA_LEN_4] = {0};
	uint8_t tmp_data1[DATA_LEN_4] = {0};
	uint8_t back_data[DATA_LEN_4] = {0};
	
	I("%s , pre-direction %d ,now direction is %d.\n",__func__,hx_s_ts->direction,direction);

//	if (hx_s_ts->direction == direction){
//		return;
//	}
	...
	switch (direction)
		{
			case PANEL_PORTRAIT_0:
				hx_parse_assign_cmd(DATA_PANEL_PORTRAIT_0_HDSK_PWD,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PANEL_PORTRAIT_0_HDSK_PWD,back_data, 4);
				hx_s_ts->direction = PANEL_PORTRAIT_0;
				break;
			case PANEL_LANDSCAPE_90:
				hx_parse_assign_cmd(DATA_PANEL_LANDSCAPE_90_HDSK_PWD,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PANEL_LANDSCAPE_90_HDSK_PWD,back_data, 4);
				hx_s_ts->direction = PANEL_LANDSCAPE_90;
				break;
			case PANEL_LANDSCAPE_180:
				hx_parse_assign_cmd(DATA_PANEL_LANDSCAPE_180_HDSK_PWD,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PANEL_LANDSCAPE_180_HDSK_PWD,back_data, 4);
				hx_s_ts->direction = PANEL_LANDSCAPE_180;
				break;
			case PANEL_LANDSCAPE_270:
				hx_parse_assign_cmd(DATA_PANEL_LANDSCAPE_270_HDSK_PWD,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PANEL_LANDSCAPE_270_HDSK_PWD,back_data, 4);
				hx_s_ts->direction = PANEL_LANDSCAPE_270;
				break;
			default :
				hx_s_ts->direction = PANEL_PORTRAIT_0;
	...
	do {
		...
		hx_s_core_fp._register_write(hx_s_ic_setup._addr_panel_direction,tmp_data,DATA_LEN_4);

		hx_s_core_fp._register_read(hx_s_ic_setup._addr_panel_direction,tmp_data1, DATA_LEN_4);
		I("%s check pwd,read value is : 0x%X%X%X%X.\n"
			,__func__,tmp_data1[3],tmp_data1[2],tmp_data1[1],tmp_data1[0]);
		retry_cnt++;
	} while ((tmp_data1[3] != back_data[3]
		|| tmp_data1[2] != back_data[2]
		|| tmp_data1[1] != back_data[1]
		|| tmp_data1[0] != back_data[0])
		&& retry_cnt < HIMAX_REG_RETRY_TIMES);
	
	if (retry_cnt >= HIMAX_REG_RETRY_TIMES)
	{
		E("%s , direction %d set fail.",__func__,hx_s_ts->direction);
		
	}
}

uint8_t himax_mcu_read_panel_direction(void)
{
	uint8_t direction = PANEL_PORTRAIT_0;
	uint8_t tmp_data[DATA_LEN_4] = {0};
	uint32_t pwd = 0;
	
	I("%s , now direction %d.",__func__,hx_s_ts->direction);	
	hx_s_core_fp._register_read(hx_s_ic_setup._addr_panel_direction,tmp_data,DATA_LEN_4);
	...
	switch (pwd)
		{
			case DATA_PANEL_PORTRAIT_0_HDSK_PWD:
				direction = PANEL_PORTRAIT_0;
				break;
			case DATA_PANEL_LANDSCAPE_90_HDSK_PWD:
				direction = PANEL_LANDSCAPE_90;
				break;				
			case DATA_PANEL_LANDSCAPE_180_HDSK_PWD:
				direction = PANEL_LANDSCAPE_180;
				break;
			case DATA_PANEL_LANDSCAPE_270_HDSK_PWD:
	...
	I("%s read direction is %d.\n",__func__,direction);
	return direction;
}
#endif
#if defined(Lenovo_game_mode_fun)
void himax_mcu_set_game_mode(uint8_t mode)
{
	uint8_t tmp_data[DATA_LEN_4],tmp_data1[DATA_LEN_4];
	...
	switch (mode)
		{
			case 0:
				hx_parse_assign_cmd(DATA_NORMAL_MODE_ENABLE,tmp_data, 4);
				hx_parse_assign_cmd(DATA_NORMAL_MODE_ENABLE,back_data, 4);
				break;
			case 1:
				hx_parse_assign_cmd(DATA_PEN_MODE_ENABLE,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PEN_MODE_ENABLE,back_data, 4);
				break;
			case 2:
				hx_parse_assign_cmd(DATA_GAME_MODE_ENABLE,tmp_data, 4);
				hx_parse_assign_cmd(DATA_GAME_MODE_ENABLE,back_data, 4);
				break;
			default :
				hx_parse_assign_cmd(DATA_NORMAL_MODE_ENABLE,tmp_data, 4);
				hx_parse_assign_cmd(DATA_NORMAL_MODE_ENABLE,back_data, 4);
		...
		I("%s mode is %d.\n",__func__,mode);
	do {
		hx_s_core_fp._register_write(hx_s_ic_setup._addr_game_mode,tmp_data, DATA_LEN_4);
		usleep_range(100, 101);		
		hx_s_core_fp._register_read(hx_s_ic_setup._addr_game_mode, tmp_data1,DATA_LEN_4);
		I("%s: tmp_data[0]=%d, mode=%d, retry_cnt=%d\n", __func__, tmp_data1[0],mode,retry_cnt);
		retry_cnt++;
	} while ((tmp_data1[3] != back_data[3]
		|| tmp_data1[2] != back_data[2]
		|| tmp_data1[1] != back_data[1]
...
uint8_t himax_mcu_read_game_mode(void)
{
	uint8_t tmp_data[DATA_LEN_4] = {0};
	uint32_t pwd = 0;
	uint8_t game_mode_tmp = 0xff;
	I("%s , now palm mode %d.",__func__,hx_s_ts->game_mode);	
	hx_s_core_fp._register_read(hx_s_ic_setup._addr_game_mode,tmp_data,DATA_LEN_4);
	pwd = (uint32_t)((tmp_data[3] << 24) | 
		   (tmp_data[2] << 16) |
		   (tmp_data[1] << 8) | 
		   (tmp_data[0]));
	...
	switch (pwd) {
		case DATA_NORMAL_MODE_ENABLE:
			game_mode_tmp = 0;
			I("%s normal mode is on \n",__func__);
			break;
		case DATA_PEN_MODE_ENABLE:
			game_mode_tmp = 1;
			I("%s PEN mode is on \n",__func__);
			break;
		case DATA_GAME_MODE_ENABLE:
			game_mode_tmp = 2;
			I("%s game mode is on \n",__func__);
			break;
		default:
	...
	if( game_mode_tmp != hx_s_ts->game_mode) {
		E("%s fw game mode state is not matching with system setting \n",__func__);
	 }

	return game_mode_tmp;
}
#endif
#if defined(Lenovo_support_pen_fun)
uint8_t himax_mcu_read_support_pen(void)
{
	uint8_t tmp_data[DATA_LEN_4] = {0};
	uint32_t pwd = 0;
	uint8_t support_pen_state;
	
	I("%s , now pen mode %d.",__func__,hx_s_ts->support_pen);	
	
	if (hx_s_ts->->suspended == false) {
		hx_s_core_fp._register_read(hx_s_ic_setup._addr_support_pen,tmp_data,DATA_LEN_4);
		pwd = (uint32_t)((tmp_data[3] << 24) | 
			   (tmp_data[2] << 16) |
			   (tmp_data[1] << 8) | 
			   (tmp_data[0]));
		...
		switch (pwd)
			{
				case DATA_SUPPORT_PEN_DISABLE:
					support_pen_state = 0;
					break;
				case DATA_SUPPORT_PEN_ENABLE:
					support_pen_state = 1;
					break;
				default :
					support_pen_state = 2;
					I("%s fw setting is in default value, need reset by baseline\n",__func__);
					break;
			}
	} else {
		support_pen_state = hx_s_ts->support_pen;
		I("%s , now is suspend mode.",__func__);	
	}
	return support_pen_state;
}

void himax_mcu_set_support_pen(uint8_t mode)
{
	uint8_t tmp_data[DATA_LEN_4],tmp_data1[DATA_LEN_4];
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
	...
	switch (mode)
		{
			case 0:
				hx_parse_assign_cmd(DATA_SUPPORT_PEN_DISABLE,tmp_data, 4);
				hx_parse_assign_cmd(DATA_SUPPORT_PEN_DISABLE,back_data, 4);
				break;
			case 1:
				hx_parse_assign_cmd(DATA_SUPPORT_PEN_ENABLE,tmp_data, 4);
				hx_parse_assign_cmd(DATA_SUPPORT_PEN_ENABLE,back_data, 4);
				break;
			default :
				hx_parse_assign_cmd(DATA_SUPPORT_PEN_DISABLE,tmp_data, 4);
				hx_parse_assign_cmd(DATA_SUPPORT_PEN_DISABLE,back_data, 4);
				E("pen mode not match, pls check!");
				break;
		}
		I("%s mode is %d.\n",__func__,mode);
	do {
		hx_s_core_fp._register_write(hx_s_ic_setup._addr_support_pen,tmp_data, DATA_LEN_4);
		usleep_range(100, 101);		
		hx_s_core_fp._register_read(hx_s_ic_setup._addr_support_pen, tmp_data1,DATA_LEN_4);
		I("%s: tmp_data[0]=%d, mode=%d, retry_cnt=%d\n", __func__, tmp_data1[0],mode,retry_cnt);
		retry_cnt++;
	} while ((tmp_data1[3] != back_data[3]
		|| tmp_data1[2] != back_data[2]
...
void himax_mcu_set_pen_id_mode(uint8_t mode)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t tmp_data1[DATA_LEN_4];
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
	...
	switch (mode)
		{
			case 0:
				hx_parse_assign_cmd(DATA_PEN_ID_RD_DIS,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PEN_ID_RD_DIS,back_data, 4);
				break;
			case 1:
				hx_parse_assign_cmd(DATA_PEN_ID_RD_EN,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PEN_ID_RD_EN,back_data, 4);
				break;
			default :
				hx_parse_assign_cmd(DATA_PEN_ID_RD_DIS,tmp_data, 4);
				hx_parse_assign_cmd(DATA_PEN_ID_RD_DIS,back_data, 4);
				E("pen id mode not match, pls check!");
				break;
		}
		I("%s pen id mode is %d.\n",__func__,mode);
	do {
		hx_s_core_fp._register_write(hx_s_ic_setup._addr_pen_id_en,tmp_data, DATA_LEN_4);
		usleep_range(100, 101);		
		hx_s_core_fp._register_read(hx_s_ic_setup._addr_pen_id_en, tmp_data1,DATA_LEN_4);
		I("%s: tmp_data[0]=%d, mode=%d, retry_cnt=%d\n", __func__, tmp_data1[0],mode,retry_cnt);
		retry_cnt++;
	} while ((tmp_data1[3] != back_data[3]
		|| tmp_data1[2] != back_data[2]
...
uint16_t himax_mcu_read_pen_id_mode(void)
{
	uint8_t tmp_data[4] = {0};
	uint8_t tmp_data_id[8] = {0};
	int m = 0,i= 0,zero_cnt = 0;
	uint32_t pwd = 0;
	uint16_t ret = DATA_RTN_AP_PEN_ID_NOT_START;
	uint16_t check_sum = 0;
	
	I("%s , now reading pen id.",__func__);	
	m = hx_s_core_fp._register_read(hx_s_ic_setup._addr_pen_id_en,tmp_data,4);
	
	if( m < 0) {
		E("%s read from IC is error.\n",__func__);
		ret = DATA_RTN_AP_PEN_ID_ERR;
		return ret;
	}
	...
	if ( pwd == DATA_PEN_ID_RD_DOING || pwd == DATA_PEN_ID_RD_EN) {
		I("%s FW is reading.\n",__func__);
		ret = DATA_RTN_AP_PEN_ID_ERR;
		return ret;
		
	}else if ( pwd == DATA_PEN_ID_RD_OVER) {
		I("%s FW had read finish.\n",__func__);
	} else {
		I("%s FW not start yet.\n",__func__);
		ret = DATA_RTN_AP_PEN_ID_NOT_START;
		return ret;
	}
	
	m = hx_s_core_fp._register_read(hx_s_ic_setup._addr_pen_id_data,tmp_data_id,8);
	if( m < 0) {
		E("%s read id data buffer is error.\n",__func__);
		ret = DATA_RTN_AP_PEN_ID_ERR;
		return ret;
	}
	
	for (i = 0; i < 8; i++) {
		check_sum += tmp_data_id[i];
		if (tmp_data_id[i] == 0x00)
			zero_cnt++;
	}
	if (check_sum % 0x100 != 0) {
		I("%s:pen id data_checksum not match check_sum=0x%02X",
			__func__,check_sum);
		ret = DATA_RTN_AP_PEN_ID_ERR;
	} else if (zero_cnt == 8) {
		I("%s:pen id data all zero",__func__);
		ret = DATA_RTN_AP_PEN_ID_ERR;
		
	} else {
		for ( i=0; i < 7;i++)
		{
			hx_s_ts->pen_id[i]= tmp_data_id[i];
			I("%s:pen id read ok",__func__);
		}
		ret = DATA_RTN_AP_PEN_ID_OK;
	}
...
#endif
#if defined(Lenovo_norch_fun)
int himax_mcu_set_prevent_notch(int32_t *data)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
	uint8_t i;
	int ret= 0;
	uint8_t buf[DATA_LEN_4] = {0};
	uint8_t tmp1,tmp2;	
	
	buf[0] = (uint8_t)(data[3] & 0xFF);
		tmp1 = (uint8_t)(data[2] & 0x0F);
		tmp2 = (uint8_t)((data[3] >> 8) & 0xFF);
	buf[1] = (tmp1 << 4) | (tmp2 & 0X0F);
	buf[2] = (uint8_t)((data[2] >> 4) & 0xFF);
	
		tmp1 = (uint8_t)(hx_s_ts->cmd_param[1] & 0xFF);
		tmp2 = (uint8_t)(hx_s_ts->cmd_param[0] & 0xFF);
	...
	do {
		 hx_s_core_fp._register_write(hx_s_ic_setup._addr_prevent_notch, buf, DATA_LEN_4);

		 hx_s_core_fp._register_read(hx_s_ic_setup._addr_prevent_notch, tmp_data, DATA_LEN_4);
