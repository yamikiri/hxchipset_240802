...
update_retry:
	...
	hx_s_core_fp._set_support_pen(hx_s_ts->support_pen);
#endif
		I("%s: TP upgrade OK\n", __func__);
	}
...
static void himax_excp_hw_reset(void)
{
	...
	hx_s_core_fp._set_panel_direction(hx_s_ts->direction);
#endif
#if defined(Lenovo_game_mode_fun)	
	hx_s_core_fp._set_game_mode(hx_s_ts->game_mode);
#endif
#if defined(Lenovo_support_pen_fun)
	hx_s_core_fp._set_support_pen(hx_s_ts->support_pen);
#endif
	...
	hx_s_core_fp._set_prevent_notch(hx_s_ts->cmd_param);	
#endif
#else
...
#endif
			if (ts->major_state == false) {
				input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR,
					g_target_report_data->p[i].w);
			} else {
				input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR,
					g_target_report_data->p[i].w + 5);
			}
#if !defined(HX_PROTOCOL_A)
...
#if defined(pen_resolution_10_en)
		ppen_info->coords.x = (g_target_report_data->s[0].x)/10;
		ppen_info->coords.y = (g_target_report_data->s[0].y)/10;
#else
...
#endif
#if defined(Lenovo_support_pen_fun)		
	hx_s_ts->support_pen = 0;
#endif
#if defined(Lenovo_norch_fun)	
	// hx_s_ts->notouch_frame = 0;
	// hx_s_ts->ic_notouch_frame = 0;
	hx_s_ts->cmd_param[0] = 0;
	hx_s_ts->cmd_param[1] = 0;
	hx_s_ts->cmd_param[2] = 0;
	hx_s_ts->cmd_param[3] = 0;
	hx_s_core_fp._set_prevent_notch(hx_s_ts->cmd_param);
#endif
#if defined(Lenovo_pen_id_fun)
	for( i=0; i< 7; i++)
